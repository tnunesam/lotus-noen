...
public class FindObject {
	...
	public List<MobileElement> findIOSElementXpath(String Xpath) {
			...
			try {
				...
				if (driver.findElements(By.xpath(Xpath)).size()> 0){
					for (WebElement element : driver.findElements(By.xpath(Xpath))) {
						elements.add((MobileElement)element);
					;
					}
				}
			} catch (Exception e) {
		...
		if (page != null) {
			...
			for (int i = 0; i < lines.length; i++) {
				if (lines[i].contains("path=" + Xpath)) {
					...
					yCenter = y + (height / 2);
					i = lines.length;
					new TouchAction((MobileDriver) driver).press(xCenter, yCenter).waitAction(1000).release().perform();
				}
			}
		} else {
...
//			}
		//	new TouchAction(driverIOS).press(x, y).perform();
			try{
			((AppiumDriver) driver).tap(1, xCenter-5, yCenter, 3000);
			}catch(Exception e){
